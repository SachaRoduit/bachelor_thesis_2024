#This script is for the xgb without eigencentrality
...
for (i in 0:16){
  ...
  #Training and test set by company ID
...
write.csv(results_matrix, file = "~/Bachelor_Arbeit/data/results_Xgb_prob_without.csv", row.names = FALSE)
...
#select optimal rounds with cross validation results
...
result_mean_xgb_without_eig = as.matrix(colMeans(result_xgb))

write.csv(result_mean_xgb_without_eig, file = "~/Bachelor_Arbeit/data/Results/result_mean_xgb_without_eig.csv", row.names = T)
write.csv(result_xgb, file = "~/Bachelor_Arbeit/data/Results/result_xgb_without_eig.csv", row.names = T)
