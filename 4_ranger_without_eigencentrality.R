#This script is for the ranger without eigencentrality
...
for (i in 0:16){
  ...
  #creating test and training set by ID splitting 
...
#Make space in the workspace(not necessary) 
rm(DF_after_date,DF_before_date,DF_Small_BD,edge_list,eigenvector_centrality,graph,l, l_split,
   l_test,l_train,summary_data,DF_startup_final,DF_startup1)

#Create a grid for a (small) tuning
...
for (i in 0:16) {
  for(j in 1:nrow(grid)) {
    ...
    pred= ifelse(m$predictions[,2] >(0.5-i*0.0066), 1, 0)
...
write.csv(results_matrix, file = "~/Bachelor_Arbeit/data/results_ranger_grid_without_eig.csv", row.names = FALSE)
...
#display results ! manually set the positive class
...
result_mean_ranger_without_eig = as.matrix(colMeans(result_ranger))

write.csv(result_mean_ranger_without_eig, file = "~/Bachelor_Arbeit/data/Results/result_mean_ranger_without_eig.csv", row.names = T)
write.csv(result_ranger, file = "~/Bachelor_Arbeit/data/Results/result_ranger_without_eig.csv", row.names = T)
